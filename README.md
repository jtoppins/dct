...
* **Game Play**
  ...
  - ticket system to track win/loss criteria
...
requests please use the issue tracker and file a new issue. Make sure
to provide as much detail about the problem or feature as possible. New
...
cd ~
apt install -y git
git clone https://github.com/jtoppins/dct.git
cd dct
./scripts/devel-setup
```

The `devel-setup` script will attempt to install additional packages that
DCT uses to run unit-tests and install code directly from the source tree.
