...
local function uiDCSEventHandler(theater, event)
	...
	if msn then
		trigger.action.outTextForGroup(grp:getID(),
			"Welcome! A mission is already assigned to this slot, use the F10 menu to get the briefing or find another!",
			20, true)
	else
		trigger.action.outTextForGroup(grp:getID(),
			"Welcome! Use the F10 Menu to get a theater update and request a mission!",
			20, true)
	end
end
