...
local RestrictedWeapons = class("RestrictedWeapons", System, DCTEvents)
...
function RestrictedWeapons:initialize()
	...
	-- TODO: register a handler with the player menu system
end
...
function RestrictedWeapons:check(agent)
	...
	local ok, totals = self:validate(agent)
	local msg = self:summary(totals)
	local header
